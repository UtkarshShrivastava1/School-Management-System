...
router.get(
  "/pending-approvals",
  verifyAdminOrTeacherToken,
  feeController.getPendingApprovals
);
router.post(
  "/:feeId/approve",
  verifyAdminOrTeacherToken,
  feeController.handleFeeApproval
);
...
router.get("/class/:classId", verifyAdminOrTeacherToken, async (req, res) => {
  try {
    ...
    allFees.forEach((fee) => {
      ...
      if (
        !latestFeesMap.has(studentId) ||
        new Date(fee.dueDate) > new Date(latestFeesMap.get(studentId).dueDate)
      ) {
...
router.post(
  "/class-fee/update",
  verifyAdminToken,
  feeController.updateClassFee
);

// Enhanced fee management routes
router.get(
  "/class/:classId/fee-history",
  verifyAdminToken,
  feeController.getClassFeeHistory
);
router.post(
...
router.get(
  "/comprehensive-stats",
  verifyAdminToken,
  feeController.getComprehensiveFeeStats
);
router.get(
  "/student/:studentId/class/:classId/fee-history",
  verifyAdminToken,
  feeController.getStudentFeeHistory
);
...
router.get(
  ...
  verifyAdminToken,
  feeController.getClassMonthlyFeeRecords
);

// Get payment history for a class
router.get(
  "/class/:classId/payment-history",
  verifyAdminToken,
  feeController.getClassPaymentHistory
);
